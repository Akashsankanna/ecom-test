...
const loadWomenProducts = async () => {
  try {
    ...
    console.log('RAW WOMEN PRODUCTS:', rawProducts)

    allWomenProducts.value = rawProducts
      .map(normalizeProduct)
      .filter((p) => p.product_id && p.variant_id)

    console.log('WOMEN PRODUCTS FINAL:', allWomenProducts.value)
  } catch (err) {
    console.error('WOMEN PRODUCTS LOAD ERROR:', err.response?.data || err)
...
const isInWishlist = (productId) => {
  const product = allWomenProducts.value.find((p) => p.id === productId)
