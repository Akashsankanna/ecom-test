...
const normalizeImageUrl = (url) => {
  ...
  // IMPORTANT: do not return doctor1.png here.
  // Otherwise every missing product image shows same image.
  if (!cleanUrl) {
    return '/no-image.png'
  }
  ...
  // If DB stores backend static path
  ...
  // If DB stores /static or static path
  ...
  // If DB stores frontend src/assets path
  ...
  // If DB stores public path
  if (cleanUrl.startsWith('/')) {
    return cleanUrl
  }

  // If DB only stores file name
  return `${getApiBaseUrl()}/uploads/products/${cleanUrl}`
}

const handleImageError = (event) => {
  if (event.target.src.includes('/no-image.png')) return
...
const getProductType = (item) => {
  ...
  if (category.includes('apron') || name.includes('apron')) return 'aprons'
...
const buildProductImages = (item) => {
  ...
  if (item.image_url) images.push(normalizeImageUrl(item.image_url))
  if (item.image) images.push(normalizeImageUrl(item.image))

  if (Array.isArray(item.images)) {
    item.images.forEach(img => {
      if (typeof img === 'string') {
        images.push(normalizeImageUrl(img))
      } else if (img?.image_url) {
        images.push(normalizeImageUrl(img.image_url))
      } else if (img?.image) {
        images.push(normalizeImageUrl(img.image))
      }
    })
  }
  ...
  if (Array.isArray(item.variants)) {
    item.variants.forEach(v => {
      if (v?.image_url) images.push(normalizeImageUrl(v.image_url))
      if (v?.image) images.push(normalizeImageUrl(v.image))
    })
  }

  const uniqueImages = [...new Set(images.filter(Boolean))]
  return uniqueImages.length ? uniqueImages : ['/no-image.png']
}

const mapBackendProduct = (item) => {
  const productImages = buildProductImages(item)

  return {
    id: Number(item.product_id || item.id),
    product_id: Number(item.product_id || item.id),
    db_product_id: Number(item.db_product_id || item.product_id || item.id),
    ...
    category: item.category_name || item.category || 'Scrubs',
    category_name: item.category_name || item.category || 'Scrubs',

    gender: item.gender || 'unisex',
    fabric: item.fabric || 'Classic',

    color: item.color || item.color_name || item.variants?.[0]?.color || '',
    color_name: item.color_name || item.color || item.variants?.[0]?.color_name || '',
    color_id: item.color_id || item.variants?.[0]?.color_id || null,
...
const loadHomeProducts = async () => {
  try {
    ...
    homeProducts.value = data
      ...
      .filter(product => product.id && product.variant_id)
...
const getVariantId = (product) => {
  return product.variant_id || product.db_variant_id || product.default_variant_id
}
...
const loadWishlist = async () => {
  try {
    ...
    const res = await api.get('/wishlist/', {
      ...
      headers: guestUuid ? { 'guest-uuid': guestUuid } : {}
    })

    wishlistIds.value = Array.isArray(res.data)
      ? res.data.map(item => Number(item.product_id || item.frontend_id || item.id))
      : []
  } catch (err) {
...
const handleWishlist = async (product) => {
  try {
    ...
    if (!variantId) {
      console.error('variant_id missing for wishlist product:', product)
      return
    }
    ...
    if (isInWishlist(product.id)) {
      ...
      wishlistIds.value = wishlistIds.value.filter(
      ...
      wishlistIds.value.push(Number(product.id))
    }
